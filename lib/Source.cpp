...
void toDrawRect(const FrameWrap& currFrame, vector<Detection>& output, const vector<string>& className) {
	...
	for (int i = 0; i < output.size(); ++i)
	{
		...
		rectangle(currFrame.image, box, color, 3);
		rectangle(currFrame.image, Point(box.x, box.y - 5), Point(box.x + box.width, box.y), color, FILLED);
		putText(currFrame.image, className[classId].c_str(), Point(box.x, box.y), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 0));

		//Modify x and y for don't overflow from original frame.
		box.x > 20 ? box.x -= 20 : box.x = box.x;
		box.y > 20 ? box.y -= 20 : box.y = box.y;
		writeRectOnDB(currFrame, box, className[classId]);
	}
}

void writeRectOnDB(const FrameWrap& currFrame, Rect rect, string objectType) {

	Mat imgFromRect = currFrame.image(rect);
